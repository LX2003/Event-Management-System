...
public class OrganizerDashboardView extends VBox {
    ...
    public OrganizerDashboardView(Stage stage, Organizer organizer) {
        ...
        exportBtn.setOnAction(e -> {
        	...
        	if (selected != null) {
        	try {
        	String eventId = String.valueOf(selected.getEvent_id());
        	String url = "http://localhost/ems-api/exportParticipants.php?event_id=" + eventId;
        	Desktop.getDesktop().browse(new URI(url));
        	} catch (Exception ex) {
        	ex.printStackTrace();
        	}
        	} else {
        	Alert alert = new Alert(Alert.AlertType.WARNING, "Please select an event from either Upcoming or Past Events.");
        	alert.showAndWait();
        	}
        	});
